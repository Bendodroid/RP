...
class Player(units.unit.Unit):
  ...
  def attack(self, target):
    ...
    if target.health <= 0:
      ...
      if self.location.name == "dungeon":
        self.location.connections.append(engine.location_handler.LocationHandler.generate_dungeon())
    else:
  ...
  def walk(self):
    ...
    for location_id in self.location.connections:
      location = engine.location_handler.LocationHandler.get_location_by_id(location_id)
